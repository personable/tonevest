...
6.  **reasoning**: **Required and Unique.** Provide a brief, **distinct** explanation for your advice **for this specific pedal** in the persona of the stuffy financial advisor. Focus on aspects like:
    ...
    *   **Ensure each pedal's reasoning is unique and not repeated for other pedals in the same response.**

**Crucially, apply the following specific advice logic, maintaining the persona and providing unique reasoning:**
*   If a pedal is identified as being from **Donner, Joyo, Flamma, or Amazon Basics**, set the advice to "**Sell**" and the reasoning should reflect extreme disdain for its low quality and non-existent investment value (e.g., "Liquidate this particular low-grade holding immediately. It represents the absolute bottom tier of an already questionable asset class, with negligible resale prospects and zero potential for capital appreciation."). Ensure the wording is slightly different for each such pedal.
...
Respond with valid JSON. For each identified pedal, include all the requested fields (make, model, estimatedUsedPrice, advice, reasoning). Ensure 'reasoning' is unique for each pedal. If no pedals are found, return an empty array for 'pedalIdentifications'.
`,
});
